...
public class ReadBookActivity extends MBaseActivity<ReadBookContract.Presenter> implements ReadBookContract.View, View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        isFirstRead = preferences.getBoolean(getString(R.string.pk_first_read), true);
...
//            }
        ...
        tvUrl.setOnClickListener(view -> {
            ...
            tv.setTextColor(ContextCompat.getColor(this, R.color.color_556270));
            tv.setTextSize(14);
            tv.setText(String.format("%s 点击确定跳转", tvUrl.getTag()));
            tv.setPadding(Utils.dp2Px(24), Utils.dp2Px(12), Utils.dp2Px(24), 0);
            AlertDialog dialog = new AlertDialog.Builder(this)
                    .setTitle(getString(R.string.dialog_title))
                    .setView(tv)
                    .setPositiveButton(getString(R.string.dialog_confirm), (dialog1, which) -> {
                        try {
                            String url = String.valueOf(tvUrl.getTag());
                ...
                new PageLoader.Callback() {
                    ...
                    public void onChapterChange(int pos) {
                        ...
                        if (mPresenter.getBookShelf().getChapterListSize() > 0) {
                            ...
                            tvUrl.setTag(NetworkUtils.getAbsoluteURL(mPresenter.getBookShelf().getBookInfoBean().getChapterUrl(),
                    ...
                    public void onPageChange(int chapterIndex, int pageIndex, boolean resetReadAloud) {
                        ...
                        autoPage();
                        if (isFirstRead) {
                            llReadGestures.setVisibility(View.VISIBLE);
                            llReadGestures.setOnClickListener(v -> {
                                isFirstRead = false;
                                SharedPreferences.Editor editor = preferences.edit();
                                editor.putBoolean(getString(R.string.pk_first_read), isFirstRead);
                                editor.apply();
                                llReadGestures.setVisibility(View.GONE);
                            });
                        }
                    }
                }
...
//                    .setPositiveButton((start, end) -> mPresenter.addDownload(start, end)).show();
            if (offlineCachingText == null || offlineCachingText.length == 0) {
                offlineCachingText = getResources().getStringArray(R.array.offline_caching);
            }
            SpannableStringBuilder builder = new SpannableStringBuilder(getString(R.string.dialog_cancel));
            ForegroundColorSpan redSpan = new ForegroundColorSpan(ContextCompat.getColor(this, R.color.color_949EA8));
            builder.setSpan(redSpan, 0, builder.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            AlertDialog dialog = new AlertDialog.Builder(this)
                    .setTitle(getString(R.string.cache_offline_caching))
                    .setNegativeButton(builder, null)
                    .setPositiveButton(R.string.dialog_confirm, (dialog1, which) -> {
                        int nowIndex = mPresenter.getBookShelf().getDurChapter();
                        int endIndex = mPresenter.getBookShelf().getChapterListSize() - 1;
                        if (downloadIndex == 0) {
                            mPresenter.addDownload(nowIndex, nowIndex + 50);
                        } else if (downloadIndex == 1) {
                            mPresenter.addDownload(nowIndex, endIndex);
                        } else {
                            mPresenter.addDownload(1, endIndex);
                        }
                    })
                    .setSingleChoiceItems(offlineCachingText, 0, (dialog2, which) -> downloadIndex = which)
                    .create();
            dialog.setOnDismissListener(dialog12 -> downloadIndex = 0);
            dialog.show();
            ATH.setAlertDialogTint(dialog);
        }
    }
...
//        }
    ...
    public void finish() {
        ...
        if (!AppActivityManager.getAppManager().isExist(MainActivity.class)) {
